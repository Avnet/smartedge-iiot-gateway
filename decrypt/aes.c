...
#define AES_STATECOLS  4
#define AES_KEYCOLS 4
#define AES_KEY_LENGTH 16
#define AES_ROUNDS 10

#define AES_EXPAND_KEY_LENGTH 4*AES_STATECOLS*(AES_ROUNDS + 1)
/* forward s-box */
...
int main()
{
  FILE *fptr;
	...
	uint8_t out[AES_EXPAND_KEY_LENGTH];
	uint8_t in[AES_EXPAND_KEY_LENGTH];
	
  aes_expand_key(ScratchArea, key_exp);
  fptr = fopen("platform_auth.key", "r");
  fread(in, 1, 16, fptr);
  fclose(fptr);
  
  aes_decrypt(in,key_exp,out);
  fptr = fopen("decode.txt", "w");
  fwrite(out, 1, 16, fptr);
  fclose(fptr);
  return 0;
}
