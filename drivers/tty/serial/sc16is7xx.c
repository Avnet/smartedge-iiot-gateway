...
static void sc16is7xx_init_rs485(struct uart_port *port);
...
{
        const u32 mask = SC16IS7XX_EFCR_AUTO_RS485_BIT | SC16IS7XX_EFCR_RTS_INVERT_BIT | SC16IS7XX_EFCR_9BIT_MODE_BIT;
  
	u32 efcr = SC16IS7XX_EFCR_9BIT_MODE_BIT;
...
static unsigned int sc16is7xx_get_mctrl(struct uart_port *port)
{
	 ...
	 */
	return TIOCM_DSR | TIOCM_CAR| TIOCM_RTS| TIOCM_CTS;
}
...
static void sc16is7xx_shutdown(struct uart_port *port)
{
	...
	/* Disable TX/RX */
	sc16is7xx_port_update(port, SC16IS7XX_EFCR_REG,
	    SC16IS7XX_EFCR_RXDISABLE_BIT | SC16IS7XX_EFCR_TXDISABLE_BIT,
            SC16IS7XX_EFCR_RXDISABLE_BIT | SC16IS7XX_EFCR_TXDISABLE_BIT);
...
static void sc16is7xx_init_rs485(struct uart_port *port)
{
	...
	if (of_get_property(np, "rs485-rts-after-send", NULL))
          {
            printk("sc16 rts after send\n");
		rs485conf->flags |= SER_RS485_RTS_AFTER_SEND;
          }
	if (of_get_property(np, "linux,rs485-enabled-at-boot-time", NULL))
          {
		rs485conf->flags |= SER_RS485_ENABLED;
                printk("sc16 rs485 enabled\n");
          }
        rs485conf->flags = SER_RS485_RTS_AFTER_SEND | SER_RS485_ENABLED;
        sc16is7xx_config_rs485(port, &port->rs485);
}
			   ...
			   struct regmap *regmap, int irq, unsigned long flags)
{
	...
	for (i = 0; i < devtype->nr_uart; ++i) {
		...
		/* Disable TX/RX */
		sc16is7xx_port_update(&s->p[i].port, SC16IS7XX_EFCR_REG,
				     SC16IS7XX_EFCR_AUTO_RS485_BIT|SC16IS7XX_EFCR_RTS_INVERT_BIT|SC16IS7XX_EFCR_9BIT_MODE_BIT, SC16IS7XX_EFCR_AUTO_RS485_BIT|SC16IS7XX_EFCR_RTS_INVERT_BIT|SC16IS7XX_EFCR_9BIT_MODE_BIT);
		...
		/* Restore access to general registers */
                //sc16is7xx_init_rs485(&s->p[i].port, dev);
